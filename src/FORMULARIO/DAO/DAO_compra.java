...
public class DAO_compra {
    ...
    public void actualizar_tabla_compra(Connection conn, JTable tbltabla,String filtro) {
        String sql = "select c.idcompra as idc, \n"
                + "to_char(c.fecha_creado,'yyyy-MM-dd') as fec_crea,\n"
                + "c.fecha_compra as fec_compra,\n"
                + "p.razon_social as proveedor,p.direccion,p.ruc,p.telefono,\n"
                + "trim(to_char(c.monto_compra,'999G999G999')) as monto,\n"
                + "c.estado \n"
    ...
    public void ancho_tabla_compra(JTable tbltabla) {
        int Ancho[] = {5,7,8,25,20,10,10,8,7};
        evejt.setAnchoColumnaJtable(tbltabla, Ancho);
    }
    public void anular_update_compra(Connection conn, compra com) {
        String titulo = "update_compra";
        String sql="UPDATE compra SET estado=? WHERE idcompra=?;";
        PreparedStatement pst = null;
        try {
            pst = conn.prepareStatement(sql);
            pst.setString(1, com.getC8estado());
