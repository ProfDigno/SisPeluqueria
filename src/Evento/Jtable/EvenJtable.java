...
//            JOptionPane.showMessageDialog(tabla, "SELECCIONAR UNA FILA PARA ELIMINAR");
    ...
    public void mostrar_JTabbedPane(JTabbedPane jtabee, int panel) {
        try {
            ...
            JOptionPane.showMessageDialog(jtabee,"ES POSIBLE QUE EL NUMERO DEL PANEL SEA INCORRECTO\nPANEL:"+panel+"\nERROR:"+ e);
        }
        
    }
    public long getLong_sumar_tabla(JTable tblitem, int columna) {
        long total_item = 0;
            ...
            if (isMayorColumna(tblitem, columna)) {
                ...
                if (isNumeric(textar)) {
                    for (int row = 0; row < tblitem.getRowCount(); row++) {
                        ...
                        long DOsubtotal = Long.parseLong(subtotal);
    ...
    public void ordenar_tabla_desc_asc(JTable tbltabla, int columnIndexToSort) {
        //solo para una columna
        TableRowSorter<TableModel> sorter = new TableRowSorter<TableModel>(tbltabla.getModel());
