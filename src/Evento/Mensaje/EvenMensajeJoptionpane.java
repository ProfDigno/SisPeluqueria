...
//    ClaDirectorio cldir=new ClaDirectorio();
    ...
    public void mensaje_error_libre(Exception e, String sql, String titulo) {
        setCant_error(getCant_error() + 1);
        String sumasql = "";
        for (int caracter = 1; caracter <= sql.length(); caracter++) {
            sumasql = sumasql + sql.substring((caracter - 1), caracter);
...
//            iconoGuardar = new ImageIcon(getClass().getResource("Evento/Mensaje/guardar.png"));
    ...
    public void error_boton_guardar(){
        JOptionPane.showMessageDialog(null, "NO SE PUEDE GUARDAR\nSE DEBE PRESIONAR EL BOTON NUEVO ANTES", 
                    "ERROR GUARDAR", JOptionPane.ERROR_MESSAGE);
    }
    public void error_boton_modificar(){
        JOptionPane.showMessageDialog(null, "NO SE PUEDE MODIFICAR\nSE DEBE SELECCIONAR LA TABLA ANTES", 
                    "ERROR MODIFICAR", JOptionPane.ERROR_MESSAGE);
    }
    /**
