...
//    private static String v7_observacion = "0";
    ...
    private void cargar_datos_venta(Connection conn, int idventa) {
        ...
        String sql = "select v.idventa,to_char(v.fecha_creado ,'yyyy-MM-dd HH24:MI') as fecha,\n"
                + "(c.idcliente||'-'||c.nombre) as cliente,\n"
                + "c.telefono,c.direccion,\n"
                + "TRIM(to_char(v.monto_total ,'999G999G999')) as monto_total,\n"
                + "TRIM(to_char(iv.cantidad,'999')) as cantidad,"
                + "iv.descripcion,\n"
        ...
        try {
            ...
            while (rs.next()) {
                ...
                v6_monto = rs.getString("monto_total");
